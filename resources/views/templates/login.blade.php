...
<div class="loginWrapper !mt-12 w-full">
    <div class="loginMain flex flex-col sm:flex-row">
        <div class="loginLeft w-full sm:w-1/2 text-6xl uppercase font-bold tracking-[3px] flex justify-center items-center">
        </div>
        <div class="loginRight w-full sm:w-1/2">
            <div class="loginFormWrapper p-4 rounded-lg max-w-420 mx-auto shadow-lg">
                <form action="{{Route('loginUser')}}" method="post" class="loginForm font-bold">
                    @csrf
                    <div class="FormTitle text-2xl sm:text-4xl uppercase border-b-2 border-gray pb-2 text-center">Login</div>
                    <div class="loginFormFields flex flex-col my-8 mx-4">
                        <label for="email" class="text-sm sm:text-xl mb-2">Enter Your Email:</label>
